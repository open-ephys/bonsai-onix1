...
{
    /// <summary>
    /// Starts data acquisition and frame distribution on a <see cref="OnixContextTask"/>.
    /// </summary>
    /// <remarks>
    ...
    /// required actions on one or more <see cref="OnixContextTask">ContextTasks</see> provided in its input
    /// sequence. Once acquisition is started, devices managed by a particular <see cref="OnixContextTask"/> will
    ...
    /// <term>Tkey</term>
    /// <description>
    /// Is the address of a particular hardware device within a single <see cref="OnixContextTask"/>.
    /// </description>
    /// </item>
    /// <item>
    /// <term>TElement</term>
    ...
    public class StartAcquisition : Combinator<OnixContextTask, IGroupedObservable<uint, oni.Frame>>
    {
        int readSize = 8192;

        /// <summary>
        /// Gets or sets the number of bytes read per cycle of the <see cref="OnixContextTask"/>'s acquisition
        /// thread.
        /// </summary>
        /// <remarks>
        /// <para>
        ...
        /// cref="OnixContextTask.MaxReadFrameSize"/>, will provide the lowest response latency, so long as data
        ...
        /// size of this option is determined by <see cref="OnixContextTask.MaxWriteFrameSize"/>. The effect on
        /// real-time performance is typically not as large as that of <see cref="OnixContextTask.BlockReadSize"/>
        ...
        /// <summary>
        /// Starts data acquisition and frame distribution on a <see cref="OnixContextTask"/> and returns the
        /// sequence of all received <see cref="oni.Frame"/> objects, grouped by device address.
        /// </summary>
        /// <param name="source">
        /// The sequence of <see cref="OnixContextTask"/> objects on which to start data acquisition and frame
        /// distribution.
        /// </param>
        /// <returns>
        /// A sequence of data frames produced by each <see cref="OnixContextTask"/> in the input sequence and
        /// grouped by device address.
        /// </returns>
        public override IObservable<IGroupedObservable<uint, oni.Frame>> Process(IObservable<OnixContextTask> source)
        {
