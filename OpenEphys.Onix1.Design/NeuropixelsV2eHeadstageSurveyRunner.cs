    ...
    internal static class NeuropixelsV2eHeadstageSurveyRunner
    {
            ...
            CancellationToken cancellationToken)
        {
            Task.Run(() => RunAsync(
                ...
                spikeThreshold, useBandpassFilter, timePerBankSeconds, recordSurveyData,
            ...
            CancellationToken cancellationToken)
        {
            ...
            static void Complete(TargetRunState state, double spikeThreshold, bool useBandpassFilter, float timePerBankSeconds)
            {
                ...
                survey.Results = new NeuropixelsV2eSurveyResults(
                    ...
                    (float)spikeThreshold, useBandpassFilter, timePerBankSeconds, state.Target.Dialog.SurveyBanks);
                    ...
                    foreach (var state in roundTargets)
                    {
                        ...
                        if (bankGroup.ChannelMap.Count == 0)
                        {
                            ...
                            if (state.NextIndex >= state.OrderedBanks.Count)
                                Complete(state, spikeThreshold, useBandpassFilter, timePerBankSeconds);
                            continue;
                        }
                    ...
                    for (int attempt = 0; ; attempt++) // only exit via Cancel or break
                    {
                        ...
                        try
                        {
                            roundResults = await CollectRoundStatsAsync(
                                ...
                                appliedVoltage ?? portVoltageOverride, spikeThreshold, useBandpassFilter,
            ...
            Action<double?> onAppliedVoltage)
        {
            ...
            foreach (var (target, deviceName, recordingFilePath) in active)
                dataSubs.Add(SubscribeTarget(deviceName, tcsMap[target], spikeThreshold, useBandpassFilter,
